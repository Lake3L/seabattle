...
public class PositionAndMove {
    // size of fields is NxN
    static final int N = 10;
    // number of coordinate data - to store i and j position
    static final int K = 2;

    public PositionAndMove() {
        setAllDirect();
        curDir = Toward.NONE;
    }
    ...
    Toward curDir;
    //current direction for hitting WILL BE HERE!!! * ... *
...
//      @return true if player hit some ship, else false
    public boolean hitEnemy(int ip, int jp) {
...
//         * @return Pair contained i and j pos of cell.
        private Pair getCoordinateToHit () {
            ...
            while (curEnemyHit.empty()) {
                p.i = r.nextInt(PositionAndMove.N);
                p.j = r.nextInt(PositionAndMove.N);
                if (player[p.i][p.j] != ATTACKED &&
